...
```mermaid
flowchart LR
    Dev[Developpeur] --> Repo[Depot GitHub (HelloWorld.java hello.py Jenkinsfile)]
    Repo --> Jenkins[Jenkins Server]
    Jenkins --> Pipeline[Execution du Jenkinsfile]
    Pipeline --> Sorties[Resultats Java et Python]
```

Ce diagramme résume parfaitement ce laboratoire :

* vous poussez les fichiers dans GitHub
* Jenkins les récupère
* la pipeline exécute le code
